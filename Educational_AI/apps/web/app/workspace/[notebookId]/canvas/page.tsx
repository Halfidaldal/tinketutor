      ...
      <div style={{ height: '100%', display: 'grid', placeItems: 'center', padding: '2rem' }}>
        <div className="surface" style={{ maxWidth: 520, padding: '2rem 2.1rem', display: 'grid', gap: '0.75rem' }}>
          ...
          <div style={{ fontSize: '0.92rem', color: 'var(--color-text-secondary)', lineHeight: 1.65 }}>
            The canvas is generated from notebook-ready evidence. Upload and process at least one source before creating a concept map.
          </div>
        </div>
      </div>
    );
      ...
      <div style={{ height: '100%', display: 'grid', placeItems: 'center', padding: '2rem' }}>
        <div className="surface" style={{ maxWidth: 560, padding: '2rem 2.1rem', display: 'grid', gap: '1rem' }}>
          <div style={{ display: 'grid', gap: '0.35rem' }}>
            <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--color-text-primary)' }}>
              Generate the notebook concept map
            </div>
            <div style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)', lineHeight: 1.65 }}>
              The first pass is deterministic, notebook-scoped, and evidence-backed. Concepts and relationships with weak support are marked tentative rather than asserted confidently.
            </div>
          </div>
          {error && (
          ...
          >
            {working ? 'Generating…' : 'Generate Concept Map'}
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div style={{ height: '100%', display: 'grid', placeItems: 'center', padding: '2rem' }}>
        <div className="surface" style={{ maxWidth: 600, padding: '2rem 2.1rem', display: 'grid', gap: '1rem' }}>
          <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--color-text-primary)' }}>
            Concept map generation could not be grounded
          </div>
          <div style={{ fontSize: '0.9rem', color: 'var(--color-text-secondary)', lineHeight: 1.65 }}>
            {conceptMap.insufficiency_reason || 'The notebook does not yet contain enough grounded evidence to assemble a concept map.'}
          </div>
          {error && (
            <div style={{ fontSize: '0.8rem', color: 'var(--color-error)' }}>
              {error}
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            ...
            >
              {working ? 'Regenerating…' : 'Try Again'}
            </button>
            <button
              type="button"
