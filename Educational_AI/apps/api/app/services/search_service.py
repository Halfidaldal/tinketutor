...
from app.services import citation_service, notebook_service, text_quality_service
...
def _lexical_candidates(
    ...
    *,
    ...
    for chunk in chunks:
        ...
        if source is None:
            continue
        if not text_quality_service.looks_readable_text(chunk.content):
            continue
...
async def build_evidence_pack(
    *,
    ...
    readable_chunks = [
        ...
        if text_quality_service.looks_readable_text(chunk.content)
    ]
    unreadable_chunk_count = len(chunks) - len(readable_chunks)
    ...
    vector_results: list[ScoredChunk] = []
    if settings.retrieval_enable_vector and any(chunk.embedding for chunk in readable_chunks):
    ...
    unreadable_candidates: list[dict] = []
            ...
            unreadable_candidates.append(
                {
                    ...
                    "reason": "Chunk text is unreadable and was excluded from evidence.",
                }
            )
            continue

        anchors = _list_anchors_for_chunk(candidate["source_id"], candidate["chunk_id"])
        snippet_text = anchors[0].snippet_text if anchors else chunk.content
        if not text_quality_service.looks_readable_text(snippet_text):
            unreadable_candidates.append(
                {
                    "chunk_id": candidate["chunk_id"],
                    "source_id": candidate["source_id"],
                    "reason": "Citation snippet text is unreadable and was excluded from evidence.",
                }
            )
            continue
    ...
    evidence_pack.diagnostics = {
        "ready_source_count": len(ready_sources),
        "chunk_count": len(chunks),
        "readable_chunk_count": len(readable_chunks),
        "unreadable_chunk_count": unreadable_chunk_count,
        ...
        "excluded_unreadable": unreadable_candidates,
    }

    if not evidence_items:
        if missing_traceability:
            evidence_pack.support_assessment = SupportAssessment.MISSING_TRACEABILITY
        elif unreadable_candidates or (unreadable_chunk_count > 0 and not readable_chunks):
            evidence_pack.support_assessment = SupportAssessment.INSUFFICIENT_GROUNDING
        else:
            evidence_pack.support_assessment = SupportAssessment.NO_MATCHING_EVIDENCE
        ...
        evidence_pack.insufficiency_reason = (
            ...
            if missing_traceability
                ...
                "Matching chunks were found, but their extracted text is unreadable. Reprocess the source with the improved PDF parser."
                if unreadable_candidates or (unreadable_chunk_count > 0 and not readable_chunks)
