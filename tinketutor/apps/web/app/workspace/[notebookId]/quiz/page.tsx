...
export default function QuizPage() {
  ...
  const fetchItems = useCallback(async (preferredBatchId?: string | null) => {
    try {
      ...
      setListError(error instanceof Error ? error.message : t('quizPage.loadError'));
    } finally {
      setLoading(false);
    }
  }, [notebookId, t]);
  ...
  const handleGenerate = useCallback(async (topic?: string) => {
    if (!hasReadySources) {
      setGenerationError(t('quizPage.needSource'));
      return;
    }
    ...
    try {
      ...
      setGenerationError(error instanceof Error ? error.message : t('quizPage.startError'));
    }
  }, [hasReadySources, notebookId, readySourceIds, t]);
    ...
    const intervalId = window.setInterval(async () => {
      try {
        ...
        if (job.status === 'failed') {
          ...
          setGenerating(false);
          setGenerationJobId(null);
          setGenerationError(job.error_message || t('quizPage.generationFailed'));
        }
      } catch (error) {
        if (cancelled) {
          return;
        ...
        setGenerating(false);
        setGenerationJobId(null);
        setGenerationError(error instanceof Error ? error.message : t('quizPage.pollError'));
      }
    }, 1000);
  ...
  return (
    <div style={{ padding: '1.35rem 1.5rem 2rem', maxWidth: 860, margin: '0 auto', display: 'grid', gap: '1rem' }}>
      ...
      >
        <div style={{ display: 'grid', gap: '0.3rem' }}>
          ...
          {activeTopic && (
            <div style={{ fontSize: '0.76rem', color: 'var(--color-text-tertiary)' }}>
              {t('quizPage.focusTopic', { values: { topic: activeTopic } })}
            </div>
          )}
        </div>
        ...
        >
          {generating
            ? t('quizPage.generatingProgress', { values: { progress: jobProgress } })
            : activeTopic
              ? t('quizPage.generateFocused')
              : t('quizPage.generate')}
        </button>
      </div>
      ...
      {generating && (
        <div className="surface" style={{ padding: '1rem 1.1rem', display: 'grid', gap: '0.35rem' }}>
          <div style={{ fontSize: '0.86rem', fontWeight: 600, color: 'var(--color-text-primary)' }}>
            {activeTopic
              ? t('quizPage.focusedGenerating', { values: { topic: activeTopic } })
              : t('quizPage.defaultGenerating')}
          </div>
          <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)', lineHeight: 1.55 }}>
            {t('quizPage.generatingBody')}
          </div>
          <div
            style={{
              marginTop: '0.25rem',
      ...
      {hasReadySources && !generating && visibleItems.length === 0 && (
        <div className="surface" style={{ padding: '2.1rem', textAlign: 'center' }}>
          <div style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--color-text-primary)', marginBottom: '0.45rem' }}>
            {activeTopic
              ? t('quizPage.emptyFocusedBatchTitle', { values: { topic: activeTopic } })
              : t('quizPage.emptyBatchTitle')}
          </div>
          <p style={{ maxWidth: 500, margin: '0 auto', color: 'var(--color-text-secondary)', fontSize: '0.9rem', lineHeight: 1.65 }}>
            {activeTopic
              ? t('quizPage.emptyFocusedBatchBody')
              : t('quizPage.emptyBatchBody')}
          </p>
        </div>
      )}
          ...
          >
            <div style={{ display: 'grid', gap: '0.2rem' }}>
              <div style={{ fontSize: '0.78rem', color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                {t('quizPage.batchSummary')}
              </div>
              <div style={{ fontSize: '1rem', fontWeight: 700, color: 'var(--color-text-primary)' }}>
                {t('quizPage.correctCount', { values: { count: correctCount, total: visibleItems.length } })}
              </div>
            </div>
            <div style={{ fontSize: '0.82rem', color: 'var(--color-text-secondary)' }}>
              {t('quizPage.answeredCount', { values: { count: answeredCount, total: visibleItems.length } })}
            </div>
          </div>
