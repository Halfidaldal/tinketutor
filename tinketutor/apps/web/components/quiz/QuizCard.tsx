...
export function QuizCard({
  ...
  const difficultyLabelKey = `quizCard.difficultyLabels.${difficulty}`;
  const difficultyLabel = t(difficultyLabelKey);
  const questionTypeLabelKey = `quizCard.typeLabels.${questionType}`;
  const questionTypeLabel = t(questionTypeLabelKey);
  ...
  const handleSubmit = async (submitValue?: string) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : t('quizCard.submitError'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="surface" style={{ padding: '1.25rem', marginBottom: '1rem' }}>
      ...
      >
        {difficultyLabel === difficultyLabelKey ? difficulty : difficultyLabel} (
        {questionTypeLabel === questionTypeLabelKey ? questionType : questionTypeLabel})
      </div>
      ...
      {!feedback ? (
          ...
          {questionType === 'open_ended' && (
            <div>
              <textarea
                ...
                placeholder={t('quizCard.answerPlaceholder')}
                rows={3}
                disabled={submitting}
                style={{
                  width: '100%',
        ...
        <div style={{ marginTop: '1rem', padding: '1rem', borderRadius: 'var(--radius-md)', background: 'var(--color-bg-elevated)', border: '1px solid var(--color-border-secondary)' }}>
          <div style={{ fontWeight: 600, fontSize: '0.875rem', marginBottom: '0.5rem', 
            ...
            {(() => {
              const resultLabelKey = `quizCard.resultLabels.${feedback.result}`;
              const resultLabel = t(resultLabelKey);
              return resultLabel === resultLabelKey
          ...
          <div style={{ fontSize: '0.875rem', lineHeight: 1.5, color: 'var(--color-text-secondary)', marginBottom: '0.75rem' }}>
            <p style={{ margin: 0 }}><strong>{t('quizCard.explanation')}</strong> {feedback.explanation}</p>
          </div>

          {feedback.correct_answer && (
             <div style={{ fontSize: '0.875rem', lineHeight: 1.5, color: 'var(--color-text-secondary)', marginBottom: '0.75rem' }}>
               <p style={{ margin: 0 }}><strong>{t('quizCard.targetAnswer')}</strong> {feedback.correct_answer}</p>
             </div>
          )}

          {(feedback.matched_concepts?.length ?? 0) > 0 && (
             <div style={{ fontSize: '0.8125rem', color: 'var(--color-text-secondary)' }}>
               <p style={{ margin: 0 }}><strong>{t('quizCard.matches')}</strong> {(feedback.matched_concepts ?? []).join(", ")}</p>
             </div>
          )}
