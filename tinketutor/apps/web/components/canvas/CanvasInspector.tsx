...
const SUPPORT_LABELS: Record<string, string> = {
  strong: 'knowledgeMapInspector.supportLabels.strong',
  partial: 'knowledgeMapInspector.supportLabels.partial',
  weak: 'knowledgeMapInspector.supportLabels.weak',
};

const STATUS_LABELS: Record<string, string> = {
  skeleton: 'knowledgeMapInspector.statusLabels.skeleton',
  student_filled: 'knowledgeMapInspector.statusLabels.student_filled',
  ai_generated: 'knowledgeMapInspector.statusLabels.ai_generated',
  verified: 'knowledgeMapInspector.statusLabels.verified',
};
...
export default function CanvasInspector({
  ...
  async function handleSave() {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : t('knowledgeMapInspector.saveError'));
    } finally {
      setSaving(false);
    }
  }
    ...
    >
      ...
      >
        <div style={{ display: 'grid', gap: '0.12rem' }}>
          <span style={{ fontSize: '0.6875rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--color-text-tertiary)' }}>
            {activeSelection.type === 'node' ? t('knowledgeMapInspector.conceptInspector') : t('knowledgeMapInspector.relationshipInspector')}
          </span>
      ...
      <div style={{ padding: '1rem', overflow: 'auto', display: 'grid', gap: '0.9rem' }}>
        ...
        >
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', alignItems: 'center' }}>
                <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-primary)' }}>
                  {t('knowledgeMapInspector.completionStatus')}
                </div>
                <span
                  style={{
                    fontSize: '0.68rem',
                ...
                >
                  {t(STATUS_LABELS[activeSelection.node.status] || activeSelection.node.status)}
                </span>
              </div>
              <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', lineHeight: 1.5 }}>
                {isSkeletonNode
                  ? t('knowledgeMapInspector.skeletonBody')
                  : t('knowledgeMapInspector.completedBody')}
              </div>
              {activeSelection.node.guiding_question && (
                ...
                >
                  <strong style={{ color: 'var(--color-text-primary)' }}>{t('knowledgeMapInspector.guidingQuestion')}</strong>{' '}
                  {activeSelection.node.guiding_question}
                </div>
              )}
            </>
          ...
          <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-primary)' }}>
            {t('knowledgeMapInspector.supportStatus')}
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', lineHeight: 1.5 }}>
            {t(SUPPORT_LABELS[support] || support)}. {activeSelection.type === 'node'
              ? activeSelection.node.uncertain
                ? t('knowledgeMapInspector.conceptReviewBody')
                : t('knowledgeMapInspector.conceptGroundedBody')
              : activeSelection.edge.uncertain
                ? t('knowledgeMapInspector.edgeReviewBody')
                : t('knowledgeMapInspector.edgeGroundedBody')}
          </div>
          <div style={{ fontSize: '0.7rem', color: 'var(--color-text-tertiary)' }}>
            {citationCount === 1
              ? t('knowledgeMapInspector.citationsOne')
              : t('knowledgeMapInspector.citationsMany', { values: { count: citationCount } })}
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gap: '0.75rem' }}>
          <label style={{ display: 'grid', gap: '0.35rem' }}>
            <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>
              {activeSelection.type === 'node' ? t('knowledgeMapInspector.conceptLabel') : t('knowledgeMapInspector.relationshipLabel')}
            </span>
            <input
              value={label}
          ...
          <label style={{ display: 'grid', gap: '0.35rem' }}>
            <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>
              {t('knowledgeMapInspector.summary')}
            </span>
            <textarea
              value={summary}
              ...
              placeholder={isSkeletonNode ? t('knowledgeMapInspector.summaryPlaceholder') : undefined}
            />
          </label>
          ...
          {activeSelection.type === 'node' && (
            <label style={{ display: 'grid', gap: '0.35rem' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--color-text-secondary)' }}>
                {t('knowledgeMapInspector.studyNote')}
              </span>
              <textarea
                value={note}
                onChange={(event) => setNote(event.target.value)}
                rows={5}
                placeholder={t('knowledgeMapInspector.notePlaceholder')}
                style={{
                  width: '100%',
                  resize: 'vertical',
                  padding: '0.7rem 0.75rem',
        ...
        <button
          ...
          >
            {saving
              ? t('knowledgeMapInspector.saving')
              : isSkeletonNode
                ? t('knowledgeMapInspector.saveCompletion')
                : t('knowledgeMapInspector.saveRefinement')}
        </button>
      </div>
    </aside>
  );
